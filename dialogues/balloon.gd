...
func next(next_id: String) -> void:
	# Если id пустой, значит это конец
	if next_id.is_empty():
		dialogue_line = null
		return

	var next_line = await dialogue_resource.get_next_dialogue_line(next_id, temporary_game_states)
...
func _on_balloon_gui_input(event: InputEvent) -> void:
	if not is_instance_valid(dialogue_resource) or not is_instance_valid(dialogue_line): 
		return
	if Global.is_autoplay:
		return
...
func _on_dialogue_label_finished_typing() -> void:
	if Global.is_autoplay:
		...
		# Проверяем, не закрылся ли диалог за эти 2 секунды
